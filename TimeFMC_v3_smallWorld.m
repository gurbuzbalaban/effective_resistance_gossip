% 1 for Minimum Resistance  % 2 for subgraident
% Comparison of Computation Times for Minimum Resistances
AverageTime=[];
AccuracyData={};
 IterationData={};
 global vect
...
Lap={};
for n=vect
    medge=floor(0.2*(n^2-n));
    [L,degree]=smallworld_graph(n,medge);
    n=n/2;
    Adj=diag(diag(L))-L;
    EDGE=Adj;
    m=0.5*sum(sum(Adj));
    ...
    % Comparison of Computation Times for FMC
    cvx_begin sdp
    variable s
    variable P(2*n,2*n)
    minimize(s)
    ...
    all_count=0;
    gk_count=0;
    while error>eps && iter<5*10^2   % Iteration of Subgradient step k
        vk=rand(2*n,1);
        vk=vk/norm(vk);
        [Ueig,eigs]=eig(Wsub);
        eigs=diag(eigs);
        ...
        while error>eps/5 && Iteration_gen<=2*n*10^6 % Second eigenvector    
            vk=Wsub*vk;
        ...
        u=vk;
% % Real Eigenvector
          alpha=10/iter; % alpha at barbell 
%           [Usub,Vsub]=eig(Wsub);
%           u=Usub(:,2*n-1);
         for i = 1:2*n
            Mi=edges(:,1)==i; % This collects the locations of Ni in edges
...
% save('ComparisonResults_FMC','Lap','-append')
